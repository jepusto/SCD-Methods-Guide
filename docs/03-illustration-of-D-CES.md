...
\end{bmatrix}$.
We define the design-comparable effect size exactly as in Equation \@ref(eq:delta-M1), because the effect size is scaled by the SD of the outcome in the absence of intervention and not dependent on the addition of $u_{1j}$, which only impacts between-case variance in the treatment phase.
